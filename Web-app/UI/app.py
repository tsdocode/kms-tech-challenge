...
def main():
    ...
    model = st.sidebar.selectbox("Select model version", ("baseline",))
    ...
    if uploaded_file is not None:
        ...
        columns_selection = st.sidebar.multiselect(
            "Select columns that you want to query", columns, default=list(columns[:3]),
        )
        ...
        # User enter question
        question = st.text_input(label="Enter your question", value="",)
        ...
        if button:
            if checkColumns(columns_selection):
                ...
                selected_dict = {
                    ...
                    if i in columns_selection
                }

                # Get SQL
                result = get_answer(
                    data=selected_dict, question=question, settings=settings
                )
